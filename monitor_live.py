...
def monitor_grid(token, duration_seconds=30):
    ...
    while time.time() - start_time < duration_seconds:
        ...
        try:
            # Get grid overview
            grid_resp = requests.get(f"{SCADA_URL}/grid/overview", headers=headers, timeout=5)
            grid = grid_resp.json() if grid_resp.status_code == 200 else {}
            
            # Get nodes
            nodes_resp = requests.get(f"{SCADA_URL}/nodes", headers=headers, timeout=5)
            ...
            # Get active alarms
            alarms_resp = requests.get(f"{SCADA_URL}/alarms/active", headers=headers, timeout=5)
            alarms = alarms_resp.json() if alarms_resp.status_code == 200 else []
            
            # Get security connections
            sec_resp = requests.get(f"{SCADA_URL}/security/connections", headers=headers, timeout=5)
            ...
            print("🔴 LIVE GRID MONITORING - Real-Time Dashboard")
            print("="*80)
            print(f"Iteration: {iteration} | Time: {datetime.now().strftime('%H:%M:%S')}\n")
            ...
            print(f"  Generation:       {grid.get('total_generation_mw', 0):.1f} MW")
            print(f"  Load:             {grid.get('total_load_mw', 0):.1f} MW")
            print(f"  Grid Losses:      {grid.get('grid_losses_mw', 0):.1f} MW")
            print(f"  Nodes Online:     {grid.get('nodes_online', 0)}/{grid.get('nodes_total', 15)}")
            
            # Node Summary
            print("\n📊 NODE STATUS SUMMARY")
            print("-"*80)
            if nodes:
                connections_online = sum(1 for n in nodes if n.get('connected', False))
            ...
            if isinstance(connections, dict):
                ...
                print(f"  Unknown:          {unknown} 🔴" if unknown > 0 else f"  Unknown:          {unknown}")
