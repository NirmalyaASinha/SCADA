...
class AttackSimulator:
    ...
    def attack_1_unauthorized_access(self):
        ...
        self.log("="*70, "ATTACK")
        self.log("Attacker: Attempting direct HTTP connection to NODE service", "ATTACK")
        ...
        try:
            ...
            if response.status_code == 401:
                ...
                self.log(f"⚠️  UNEXPECTED: Got response {response.status_code}", "WARNING")
                
        except requests.exceptions.ConnectionError:
            self.log("✅ BLOCKED: Connection refused (firewall/network isolation)", "SUCCESS")
        except Exception as e:
    ...
    def attack_2_parameter_tampering(self):
        ...
        self.log("="*70, "ATTACK")
        self.log("Attacker: Attempting to modify breaker state on SUB-001", "ATTACK")
        ...
        try:
            ...
            if response.status_code in [401, 403]:
                self.log("✅ BLOCKED: Request rejected - authentication required", "SUCCESS")
            else:
    ...
    def attack_3_token_forgery(self):
        ...
        try:
            ...
            if response.status_code == 401:
                self.log(f" Response Code: {response.status_code}", "ATTACK")
                self.log("✅ BLOCKED: Invalid token rejected by server", "SUCCESS")
                self.log("🔒 JWT signature validation: PASSED", "SUCCESS")
            else:
                self.log(f"⚠️  Unexpected response: {response.status_code}", "WARNING")
    ...
    def attack_4_mass_requests(self):
        ...
        for i in range(5):
            try:
                ...
                if response.status_code == 401:
                    blocked_count += 1
                self.log(f"  Request {i+1}: Status {response.status_code}", "ATTACK")
            except Exception as e:
                blocked_count += 1
                self.log(f"  Request {i+1}: Blocked ({type(e).__name__})", "ATTACK")
    ...
    def attack_5_privilege_escalation(self):
        ...
        self.log("="*70, "ATTACK")
        self.log("Attacker: Login as viewer, attempt engineer-level control", "ATTACK")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if sbo_response.status_code == 403:
                    self.log(f"  Response: {sbo_response.status_code}", "ATTACK")
                    self.log("✅ BLOCKED: Viewer cannot perform SBO operations", "SUCCESS")
                    self.log("🔒 RBAC enforced: viewer role has insufficient privileges", "SUCCESS")
                else:
                    self.log(f"⚠️  Unexpected response: {sbo_response.status_code}", "WARNING")
