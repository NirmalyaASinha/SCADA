...
class AdvancedAttackTest:
    ...
    def simulate_cascading_failure(self):
        ...
        self.log(
            "Scenario: Attacker compromises SUB-001, triggers cascade",
            "SCENARIO")
        self.log(
            "Goal: Isolate multiple substations and cause load shedding",
            "SCENARIO")
        ...
        grid = self.get_grid()
        self.log(
            f"✓ Grid Frequency: {grid.get('system_frequency_hz', 50):.3f} Hz (BASELINE)",
            "MONITOR")
        self.log(
            f"✓ Total Generation: {grid.get('total_generation_mw', 0):.0f} MW",
            "MONITOR")
        self.log(
            f"✓ System Load: {grid.get('total_load_mw', 0):.0f} MW",
            "MONITOR")
        ...
        self.log(
            "ATTACK: Attempting to send malicious setpoints to SUB-001",
            "ATTACK")

        malicious_payloads = [{"target": "SUB-001",
                               "action": "Force breaker open",
                               "parameter": "breaker_state",
                               "value": 0},
                              {"target": "SUB-001",
                               "action": "Disable voltage control",
                               "parameter": "auto_control",
                               "value": 0},
                              {"target": "SUB-002",
                               "action": "Force low voltage",
                               "parameter": "voltage_setpoint",
                               "value": 0.85},
                              {"target": "SUB-003",
                               "action": "Reverse power flow",
                               "parameter": "pflag",
                               "value": -1},
                              ]
        ...
        for i, payload in enumerate(malicious_payloads, 1):
            self.log(
                f"  [{i}] {payload['target']}: {payload['action']}",
                "ATTACK")
            ...
            try:
                ...
                if result.status_code >= 400:
                    self.log(
                        f"      ❌ BLOCKED (HTTP {result.status_code})",
                        "DEFENSE")
                else:
        ...
        if alarms:
            ...
            self.log(
                "✅ System blocking attacks - no anomalies detected",
                "SUCCESS")
        ...
        self.log(
            f"Grid stability: {'MAINTAINED ✅' if abs(freq_change) < 0.1 else 'DEGRADED ⚠️'}",
            "SYSTEM")
        self.log(
            f"Attack impact: {'MITIGATED' if abs(freq_change) < 0.1 else 'PARTIAL'}",
            "SYSTEM")
